...
$lid = (int)$_GET['lid'];
$cid = (int)$_GET['cid'];
$sql = sprintf('UPDATE %s SET hits = hits+1 WHERE lid = %u AND status > 0', $xoopsDB->prefix('addresses_links'), $lid);
$xoopsDB->queryF($sql);
$result = $xoopsDB->query('select url from ' . $xoopsDB->prefix('addresses_links') . " where lid=$lid and status>0");
...
$xoopsTpl->assign(
    ...
    [
        ...
        'mail_body' => rawurlencode(sprintf(_MD_INTLINKFOUND, $xoopsConfig['sitename']) . ':  ' . XOOPS_URL . '/modules/addresses/visitlink.php?lid=' . $lid),
    ]
);
