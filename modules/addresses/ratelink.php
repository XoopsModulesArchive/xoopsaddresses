...
if (!empty($_POST['submit'])) {
    ...
    if (0 != $ratinguser) {
        $result = $xoopsDB->query('select submitter from ' . $xoopsDB->prefix('addresses_links') . " where lid=$lid");
        ...
        $result = $xoopsDB->query('select ratinguser from ' . $xoopsDB->prefix('addresses_votedata') . " where lid=$lid");
        ...
        $result = $xoopsDB->query('select count(*) FROM ' . $xoopsDB->prefix('addresses_votedata') . " WHERE lid=$lid AND ratinguser=0 AND ratinghostname = '$ip' AND ratingtimestamp > $yesterday");
    ...
    $sql = sprintf("INSERT INTO %s (ratingid, lid, ratinguser, rating, ratinghostname, ratingtimestamp) VALUES (%u, %u, %u, %u, '%s', %u)", $xoopsDB->prefix('addresses_votedata'), $newid, $lid, $ratinguser, $rating, $ip, $datetime);
    ...
    $GLOBALS['xoopsOption']['template_main'] = 'addresses_ratelink.html';
    require XOOPS_ROOT_PATH . '/header.php';
    $lid = (int)$_GET['lid'];
    $cid = (int)$_GET['cid'];
    $result = $xoopsDB->query('select title from ' . $xoopsDB->prefix('addresses_links') . " where lid=$lid");
