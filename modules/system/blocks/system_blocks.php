...
function b_system_waiting_show()
{
    ...
    if ($moduleHandler->getCount(new Criteria('dirname', 'mydownloads'))) {
        ...
        // ADDED for addresses module
    }

    if ($moduleHandler->getCount(new Criteria('dirname', 'addresses'))) {
        $result = $xoopsDB->query('SELECT COUNT(*) FROM ' . $xoopsDB->prefix('addresses_links') . ' WHERE status=0');

        if ($result) {
            $block['modules'][8]['adminlink'] = XOOPS_URL . '/modules/addresses/admin/index.php?op=listNewLinks';
        ...
        if ($result) {
            $block['modules'][9]['adminlink'] = XOOPS_URL . '/modules/addresses/admin/index.php?op=listBrokenLinks';

            [$block['modules'][9]['pendingnum']] = $xoopsDB->fetchRow($result);

            $block['modules'][9]['lang_linkname'] = _MB_SYSTEM_BADRESS;
        }

        $result = $xoopsDB->query('SELECT COUNT(*) FROM ' . $xoopsDB->prefix('addresses_mod'));

        if ($result) {
            $block['modules'][10]['adminlink'] = XOOPS_URL . '/modules/addresses/admin/index.php?op=listModReq';

            [$block['modules'][10]['pendingnum']] = $xoopsDB->fetchRow($result);

            $block['modules'][10]['lang_linkname'] = _MB_SYSTEM_MADRESS;
        }
            ...
            while (false !== ($userinfo = $xoopsDB->fetchArray($result))) {
                ...
                if ('' != $xoopsUser) {
                    ...
                    if ($userinfo['user_viewemail']) {
                        ...
                        $block['groups'][$i]['users'][] = ['id' => $userinfo['uid'], 'name' => htmlspecialchars($userinfo['uname'], ENT_QUOTES | ENT_HTML5), 'msglink' => '&nbsp;', 'avatar' => XOOPS_URL . '/uploads/' . $userinfo['user_avatar']];
                    }
                }
...
function b_system_newmembers_show($options)
{
    ...
    for ($i = 0; $i < $count; $i++) {
        if (1 == $options[1]) {
            $block['users'][$i]['avatar'] = 'blank.gif' != $newmembers[$i]->getVar('user_avatar') ? XOOPS_URL . '/uploads/' . $newmembers[$i]->getVar('user_avatar') : '';
        } else {
...
function b_system_topposters_show($options)
{
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if (1 == $options[1]) {
            $block['users'][$i]['avatar'] = 'blank.gif' != $topposters[$i]->getVar('user_avatar') ? XOOPS_URL . '/uploads/' . $topposters[$i]->getVar('user_avatar') : '';
        } else {
            $block['users'][$i]['avatar'] = '';
        }
